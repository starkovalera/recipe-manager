...
- Clerk `user.deleted` currently performs only an idempotent transition to `DELETION_PENDING` and records `deletion_requested_at`. Destructive cleanup and deletion-task scheduling remain deferred to the dedicated account-deletion iteration.
