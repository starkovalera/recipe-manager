...
- Storage-key safety is provider-owned. LOCAL validates keys with the current runtime's `Path` semantics and resolved-root containment; S3 treats non-empty object keys as opaque strings. Destructive maintenance separately verifies that referenced nested keys belong to the expected domain namespace and only accepts flat keys through the legacy compatibility path.
