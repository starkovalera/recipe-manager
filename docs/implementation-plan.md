...
- `APP_ENV=PROD`
  ...
  - SQS and S3 adapters are implemented; no fallback provider is selected.
...
- SQS publishing lazily creates and reuses one boto3 client per publisher instance, routes each operation to its dedicated configured queue, sends only the validated ID-only wire message, requires a non-empty `MessageId`, and propagates SDK errors unchanged so the transactional outbox remains responsible for publication failure recovery.
- AWS SDK imports and SQS API calls are confined to the SQS queue adapter. The queue factory may select the adapter but must not access boto3 directly. The application-level `QueuePublisher` contract remains limited to import, recipe-embedding, and account-deletion publication.
...
- `DEV`, `PREVIEW`, and `TEST` retain Dramatiq-compatible local behavior. `QUEUE_PROVIDER=SQS` selects the production SQS adapter only when all required AWS settings are valid; publisher construction remains lazy and performs no AWS calls.
...
- An accepted import retry atomically persists `FAILED -> QUEUED`, its `IMPORT_STARTED` notification, and one pending `IMPORT_JOB` outbox message. Immediate dispatch happens after commit. Dispatch failure does not revert the job or delete its notification; reconciliation retries the pending message.
- Embedding scheduling atomically persists the embedding state, lifecycle event, and pending `RECIPE_EMBEDDING` outbox message. Callers dispatch only after their transaction commits. Immediate dispatch failure does not revert the completed import, recipe mutation, retry, or stale-requeue transition. `ENQUEUED` is created only by the post-publication outbox hook.
- Embedding processing claims work by locking the active recipe row in a short transaction and returns explicit `SUCCEEDED`, `NOOP`, `REQUEUED`, `BUSY`, or `RETRYABLE_FAILURE` outcomes. `RUNNING` is duplicate-safe and never starts a second provider call. Provider execution occurs outside database sessions; provider failures persist `FAILED` and increment `failed_attempts`, which counts provider failures rather than transport deliveries.
- Each accepted user-initiated account deletion request atomically persists the `DELETION_PENDING` state when first requested and a new pending `ACCOUNT_DELETION` outbox message. Repeated requests while deletion is pending create new durable intents. Immediate dispatch happens only after commit, and dispatch failure does not revert the pending account state or remove the outbox message.
- Each new verified `user.deleted` webhook atomically persists its `svix-id` idempotency record and, when the local user exists, the user's `DELETION_PENDING` state plus a pending `ACCOUNT_DELETION` outbox message. A duplicate `svix-id` creates and dispatches nothing; an unknown local user records the processed webhook without an outbox message. Immediate dispatch occurs only after commit, and dispatch failure does not change the successful webhook response or remove the pending intent.
- Manual account-deletion reconciliation creates one pending `ACCOUNT_DELETION` outbox message per `DELETION_PENDING` user in a single committed database operation, then dispatches those message IDs after commit. It reports failed user IDs through the existing contract, and `python -m app.users.reconcile_deletions` keeps a nonzero exit code when any immediate dispatch fails.
- Storage provider selection is centralized in `get_storage_service()` and does not select a logical location. Every operation names `StorageLocation.USER_MEDIA` or `StorageLocation.SYSTEM_ARTIFACTS`; LOCAL resolves locations to paths and S3 resolves them to distinct private buckets.
- Storage saves accept the shared `StorageSaveContext` protocol. User media uses `StorageUserContext` with purpose-first `imports/source/{owner}/{job}/`, `imports/derived/{owner}/{job}/`, and `recipes/media/{owner}/{recipe}/` keys. Maintenance reports use `StorageSystemContext` under `maintenance/reports/`. Keys never include a `users/` prefix or the original filename.
- Storage-key safety is provider-owned. LOCAL validates keys with the current runtime's `Path` semantics and resolved-root containment; S3 treats non-empty object keys as opaque strings. Application media and destructive maintenance accept only canonical purpose-first keys in the expected domain namespace; pre-production databases are recreated instead of carrying flat-key compatibility.
...
- Public domain responses expose stable `recipe_image` and `import_source_image` IDs, never storage keys or durable access URLs. Authenticated `POST /media/access` resolves ownership/lifecycle in bounded type batches and preserves request order, duplicates, and partial success. LOCAL grants require authenticated fetch through `GET /media/{media_type}/{media_id}` with repeated authorization; S3 grants are direct 60-second presigned GETs without `HeadObject`. Access mode describes browser mechanics rather than provider or visibility.
- Failed-import artifact cleanup operates in DB snapshot, storage I/O, and fresh DB finalization phases. It never holds a DB transaction during storage calls; only complete cleanup moves a job to non-retryable `FAILED_ARTIFACTS_REMOVED` and records `IMPORT_ARTIFACTS_REMOVED`.
- Orphan detection is read-only: it lists all pages under current user-media prefixes, resolves durable `ImportJobSource`/`RecipeImage` references in batches, and reports only old unreferenced or malformed objects. It never deletes an object.
- Maintenance JSON reports are stored only for anomalies/failures in `SYSTEM_ARTIFACTS`; clean runs emit structured logs. Required report persistence failure makes the operation retryable.
...
- **Cleanup safety:** Every cleanup path must verify domain ownership before deleting. Filesystem providers must additionally resolve keys under the configured storage root; object-storage providers apply their own key validation semantics.
