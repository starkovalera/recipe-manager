...
(async () => {
  ...
  try {
    ...
    for (const viewport of [{ width:360,height:800 },{ width:390,height:844 },{ width:430,height:900 }]) {
      ...
      assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth), true, `${viewport.width}px keeps Basics free of horizontal overflow`);
      assert.equal(await page.locator('.basics-grid').count(), 1, `${viewport.width}px renders the aligned Basics grid`);
      assert.deepEqual(await page.locator('select[name="source"] option').allTextContents(), ['Manual','Instagram','Threads','TikTok','Other'], `${viewport.width}px uses the fixed Source values`);
      assert.deepEqual(await page.locator('select[name="difficulty"] option').allTextContents(), ['Not set','Easy','Moderate','Hard'], `${viewport.width}px uses the fixed Difficulty values`);
      assert.deepEqual(await page.locator('select[name="rating"] option').allTextContents(), ['Not rated','1 out of 5','2 out of 5','3 out of 5','4 out of 5','5 out of 5'], `${viewport.width}px uses whole-step Personal rating values`);
      const basicsMetrics = await page.locator('.basics-grid > label:not(.full-width) input, .basics-grid > label:not(.full-width) select').evaluateAll(controls => {
      ...
      assert.equal(new Set(basicsMetrics.widths).size, 1, `${viewport.width}px gives every paired Basics control the same width`);
      assert.deepEqual(basicsMetrics.heights, [44,44,44,44,44,44], `${viewport.width}px gives every Basics control the same height`);
      assert.deepEqual(await page.locator('.basics-grid input, .basics-grid select').evaluateAll(controls => controls.map(control => Math.round(control.getBoundingClientRect().height))), [44,44,44,44,44,44,44], `${viewport.width}px gives title and paired Basics controls the same height`);
      await page.getByRole('button', { name:/Ingredients 12 of 50/ }).click();
      assert.equal(await page.locator('.ingredient-list .handle').count(), 0, `${viewport.width}px has no mobile ingredient reorder handles`);
      assert.equal(await page.locator('.ingredient-list [aria-label^="Reorder"]').count(), 0, `${viewport.width}px has no mobile ingredient move commands`);
      assert.equal(await page.getByText('Drag to reorder').count(), 0, `${viewport.width}px has no mobile ingredient reorder instruction`);
      assert.equal(await page.locator('.ingredient-list li').count(), 4, `${viewport.width}px keeps ingredient rows`);
      assert.equal(await page.locator('.ingredient-list .row-summary').count(), 4, `${viewport.width}px keeps summary-button ingredient editing`);
      assert.equal(await page.locator('.ingredient-list .trash').count(), 4, `${viewport.width}px keeps trash ingredient actions`);
    ...
    console.log('MOBILE_EDIT_REFINEMENT_CHECKS_PASS');
  } finally {
    await browser.close();
