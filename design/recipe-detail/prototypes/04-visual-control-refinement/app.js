(function () {
  ...
  const state = {
    ...
    pending: null, message: '', mediaSelected: 0, mediaMenuOpen: false, returnPanel: null,
    previewExpanded: new Set(), panelScroll: { import: 0, media: 0 }
  };
  ...
  function defaultHeader(recipe) {
    ...
    const wideActions = state.placement === 'full-width' ? `<div class="header-wide-actions">${actions}</div>` : '';
    return `<div class="header-zone placement-${state.placement}"><div class="cover-stack"><div class="cover ${recipe.cover ? 'has-cover' : ''}" role="img" aria-label="${recipe.cover ? 'Recipe cover placeholder' : 'No cover available'}">${recipe.cover ? 'Recipe cover' : 'No cover available'}</div></div><header class="identity"><h1>${esc(recipe.title)}</h1>${sourceRow}${factsRow}${state.placement === 'under-title' ? actions : ''}</header>${wideActions}${reviewStatus(recipe)}${secondaryMetadata(recipe)}</div>`;
  }
  ...
  function resourcePreview(resource, size) {
    ...
    return `<button type="button" class="resource-thumbnail ${size === 'primary' ? 'primary-thumbnail' : ''}" data-preview-id="${esc(resource.id)}" aria-expanded="${expanded}" aria-label="${expanded ? 'Collapse' : 'Preview'} ${esc(resource.label)}" title="${expanded ? 'Collapse preview' : 'Preview image'}"><img src="${esc(resource.preview)}" alt="" width="64" height="48"></button>`;
  }
  ...
  function resourceGroup(group) {
    ...
    const primaryBody = `<div class="resource-item-body">${resourcePreview(group, 'primary')}<div><span class="resource-type">${esc(group.type)}</span><h3 id="resource-${esc(group.id)}">${esc(group.label)}</h3><p>${esc(group.detail)} · ${esc(group.status)}${children.length ? ` · ${children.length} derived (${esc(counts)})` : ''}</p></div></div>`;
    const childRows = children.map(child => {
      const affected = pending && !child.currentCover;
      const protectedCover = pending && child.currentCover;
      return `<div class="resource-child ${affected ? 'will-remove' : ''} ${protectedCover ? 'will-keep' : ''}"><div class="resource-item-body">${resourcePreview(child)}<div><span class="resource-type">${esc(child.type)}</span><strong>${esc(child.label)}</strong><span class="resource-status">${esc(child.status)}${child.currentCover ? ' · Current cover' : ''}</span></div></div>${child.currentCover ? '<span class="cover-lock resource-consequence">Kept as cover</span>' : pending ? '<span class="pending-marker resource-consequence">Will be removed</span>' : `<button type="button" class="icon-button icon-remove" data-remove-child="${esc(child.id)}" data-type="${esc(child.type)}" data-label="${esc(child.label)}" aria-label="Remove ${esc(child.label)}" title="Remove resource">${icon('trash')}</button>`}${expandedResourcePreview(child)}</div>`;
    }).join('');
    return `<section class="resource-group ${pending ? 'pending-removal' : ''}" aria-labelledby="resource-${esc(group.id)}"><header class="resource-primary">${primaryBody}${pending ? '<span class="pending-marker">Pending</span>' : `<button type="button" class="remove-button" data-remove-primary="${esc(group.id)}" aria-label="Remove ${esc(group.label)}">Remove</button>`}</header>${expandedResourcePreview(group)}${primaryConfirmation(group)}${children.length ? `<div class="resource-children">${childRows}</div>` : ''}</section>`;
  }
  ...
  function mediaDrawer() {
    ...
    return `<aside class="layer drawer media-drawer" role="dialog" aria-modal="${window.innerWidth < 1200}" aria-labelledby="media-title" data-panel-type="media"><div class="layer-heading"><div><h2 id="media-title">Cooking media · ${mediaTotal(scenario)}</h2><span class="meta-label">Images and external cooking references</span></div><div class="layer-heading-actions"><div class="media-overflow"><button type="button" class="icon-button" data-action="toggle-media-menu" aria-label="More media actions" aria-haspopup="menu" aria-expanded="${state.mediaMenuOpen}" title="More media actions">${icon('more')}</button>${menu}</div><button type="button" data-action="close-layer">Close</button></div></div><section class="media-section" aria-labelledby="media-images-title"><h3 id="media-images-title">Images · ${media.length}</h3>${imageContent}</section><section class="media-section" aria-labelledby="media-links-title"><h3 id="media-links-title">Videos &amp; links · ${links.length}</h3>${linkContent}</section></aside>`;
  }
  ...
  function openLayer(type, trigger, details) {
    state.returnPanel = null; state.mediaMenuOpen = false;
  ...
  function closeLayer(returnFocus = true) {
    ...
    state.layer = null; state.layerTrigger = null; state.pending = null; state.returnPanel = null; state.mediaMenuOpen = false;
  ...
  root.addEventListener('click', event => {
    ...
    if (action === 'import') {
      if (state.layer?.type === 'media') { state.returnPanel = 'media'; state.mediaMenuOpen = false; switchPanel('import'); }
      else openLayer('import', event.target.closest('button'));
    }
    else if (action === 'media') {
      if (state.layer?.type === 'import') { state.returnPanel = null; switchPanel('media'); }
  ...
  layerRoot.addEventListener('click', event => {
    ...
    else if (action === 'toggle-media-menu') { state.mediaMenuOpen = !state.mediaMenuOpen; renderLayer({ focusSelector: state.mediaMenuOpen ? '[data-action="import-from-media"]' : '[data-action="toggle-media-menu"]' }); }
    else if (action === 'import-from-media') { state.returnPanel = 'media'; state.mediaMenuOpen = false; switchPanel('import'); }
    else if (action === 'back-media') { state.returnPanel = null; switchPanel('media'); }
    else if (action === 'prepare-flags') { state.pending = { type: 'flags' }; renderLayer(); }
    else if (action === 'cancel-confirm') { const groupId = state.pending?.groupId; state.pending = null; renderLayer({ focusSelector: groupId ? `[data-remove-primary="${groupId}"]` : '[data-action="prepare-flags"]' }); }
  ...
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && state.mediaMenuOpen) { state.mediaMenuOpen = false; renderLayer({ focusSelector: '[data-action="toggle-media-menu"]' }); return; }
    if (event.key === 'Escape' && state.layer && state.pending) { const groupId = state.pending.groupId; state.pending = null; renderLayer({ focusSelector: groupId ? `[data-remove-primary="${groupId}"]` : '[data-action="prepare-flags"]' }); announce('Removal cancelled.'); return; }
  ...
  scenarioSelect.addEventListener('change', () => { state.scenario = scenarioSelect.value; state.view = 'default'; state.expanded = { ingredients: false, instructions: false, notes: false }; state.removedIds.clear(); state.removedItems = []; state.previewExpanded.clear(); state.removedExpanded = false; state.mediaSelected = 0; state.mediaMenuOpen = false; state.returnPanel = null; state.panelScroll = { import: 0, media: 0 }; state.pending = null; state.message = ''; closeLayer(false); renderCurrentState(); window.scrollTo(0, 0); });
