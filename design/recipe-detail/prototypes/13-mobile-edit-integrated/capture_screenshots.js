...
(async () => {
  ...
  try {
    ...
    await page.locator('.phone').screenshot({ path:path.join(outputDir,'13a-mobile-edit-integrated-baseline-v1.png') });
    await page.locator('[data-edit-ingredient="1"]').click();
    await page.locator('.phone').screenshot({ path:path.join(outputDir,'13b-mobile-edit-integrated-ingredient-sheet-v1.png') });
    await page.getByRole('button', { name:'Close' }).click();
    await page.getByRole('button', { name:'More recipe actions' }).click();
    await page.locator('.phone').screenshot({ path:path.join(outputDir,'13c-mobile-edit-integrated-overflow-v1.png') });
    console.log('MOBILE_EDIT_INTEGRATED_SCREENSHOTS_CAPTURED');
  } finally {
    await browser.close();
