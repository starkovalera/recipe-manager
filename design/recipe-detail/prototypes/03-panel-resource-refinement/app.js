(function () {
  ...
  const state = {
    ...
    pending: null, message: '', mediaExpanded: false, mediaSelected: 0,
  ...
  function resourcePreview(resource, size) {
    ...
    return `<button type="button" class="resource-thumbnail ${size === 'primary' ? 'primary-thumbnail' : ''}" data-preview-id="${esc(resource.id)}" aria-expanded="${expanded}" aria-label="${expanded ? 'Collapse' : 'Preview'} ${esc(resource.label)}"><img src="${esc(resource.preview)}" alt=""></button>`;
  }

  function expandedResourcePreview(resource) {
    if (!state.previewExpanded.has(resource.id) || !resource.preview) return '';
    return `<div class="expanded-resource-preview"><img src="${esc(resource.preview)}" alt="Expanded preview of ${esc(resource.label)}"><button type="button" data-preview-id="${esc(resource.id)}">Close preview</button></div>`;
  }
  ...
  function resourceGroup(group) {
    ...
    const childRows = children.map(child => {
      ...
      return `<div class="resource-child ${affected ? 'will-remove' : ''} ${protectedCover ? 'will-keep' : ''}"><div class="resource-item-body">${resourcePreview(child)}<div><span class="resource-type">${esc(child.type)}</span><strong>${esc(child.label)}</strong><span class="resource-status">${esc(child.status)}${child.currentCover ? ' · Current cover' : ''}</span></div></div>${child.currentCover ? '<span class="cover-lock">Kept as cover</span>' : pending ? '<span class="pending-marker">Will be removed</span>' : `<button type="button" class="icon-remove" data-remove-child="${esc(child.id)}" data-type="${esc(child.type)}" data-label="${esc(child.label)}" aria-label="Remove ${esc(child.label)}">×</button>`}${expandedResourcePreview(child)}</div>`;
    }).join('');
  ...
  function panelSwitcher(active) {
    const scenario = current();
    if (state.view !== 'cooking' || !scenario.recipe.imported || !scenario.mediaAvailable) return '';
    return `<nav class="panel-switcher" aria-label="Cooking auxiliary panel"><button type="button" data-panel="media" ${active === 'media' ? 'aria-current="page" disabled' : ''}>Media · ${scenario.media.length}</button><button type="button" data-panel="import" ${active === 'import' ? 'aria-current="page" disabled' : ''}>Import info</button></nav>`;
  }
  ...
  function importDrawer() {
    ...
    return `<aside class="layer drawer import-drawer" role="dialog" aria-modal="${window.innerWidth < 1200}" aria-labelledby="import-title" data-panel-type="import"><div class="layer-heading"><div><h2 id="import-title">Import info</h2><span class="meta-label">Review messages and imported resources</span></div><button type="button" data-action="close-layer" aria-label="Close Import info">Close</button></div>${panelSwitcher('import')}${pendingConfirmation(info)}${state.message ? `<p class="drawer-message" role="status">${esc(state.message)}</p>` : ''}<section class="drawer-section flags-section"><div class="section-heading"><h3>Review needed · ${flagsOpen ? info.flags.length : 0}</h3>${flagsOpen ? '<button type="button" data-action="prepare-flags">Mark all reviewed</button>' : ''}</div>${flagsOpen ? `<ul class="flag-list">${info.flags.map(flag => `<li>${esc(flag)}</li>`).join('')}</ul><p class="helper-text">After reviewing these messages, edit the recipe or mark all reviewed to keep it as it is.</p><button type="button" data-action="edit-from-drawer">Edit recipe</button>` : '<p class="empty-inline">All import messages have been reviewed.</p>'}</section><section class="drawer-section"><h3>Imported resources</h3><p class="helper-text">Derived resources are grouped beneath the primary resource that produced them.</p>${info.groups.map(resourceGroup).join('') || '<p class="empty-inline">No active imported resources.</p>'}</section>${records.length ? `<section class="drawer-section removed-summary"><button type="button" class="summary-button" data-action="toggle-removed" aria-expanded="${state.removedExpanded}"><span>Removed resources · ${records.length}</span><span>${esc(countTypes(records))}</span></button>${state.removedExpanded ? `<ul>${records.map(item => `<li>${esc(item.type)} · ${esc(item.label)}</li>`).join('')}</ul><p class="helper-text">Removed resources cannot be restored here.</p>` : ''}</section>` : ''}${state.role === 'debug' ? `<section class="drawer-section"><h3>Extraction details</h3><p class="debug-detail">${esc(info.debug)}</p></section>` : ''}</aside>`;
  }

  function mediaDrawer() {
    const media = current().media;
    const selected = media[state.mediaSelected] || media[0];
    return `<aside class="layer drawer media-drawer ${state.mediaExpanded ? 'media-expanded' : ''}" role="dialog" aria-modal="${window.innerWidth < 1200}" aria-labelledby="media-title" data-panel-type="media"><div class="layer-heading"><div><h2 id="media-title">Cooking media · ${media.length}</h2><span class="meta-label">Supplementary reference</span></div><div><button type="button" data-action="expand-media">${state.mediaExpanded ? 'Compact' : 'Expand'}</button> <button type="button" data-action="close-layer">Close</button></div></div>${panelSwitcher('media')}<article class="media-feature"><img src="${esc(selected.preview)}" alt="${esc(selected.title)}"><strong>${esc(selected.title)}</strong><div class="meta-label">${esc(selected.origin)} · ${esc(selected.author)}${selected.current ? ' · Current cover' : ''}</div></article><div class="media-strip" aria-label="Choose cooking media">${media.map((item, index) => `<button type="button" data-media-index="${index}" aria-pressed="${index === state.mediaSelected}"><img src="${esc(item.preview)}" alt=""><span>${esc(item.title)}</span></button>`).join('')}</div></aside>`;
  }
  ...
  root.addEventListener('click', event => {
    ...
    if (action === 'import') state.layer?.type === 'media' ? switchPanel('import') : openLayer('import', event.target.closest('button'));
    else if (action === 'media') state.layer?.type === 'import' ? switchPanel('media') : openLayer('media', event.target.closest('button'));
  ...
  layerRoot.addEventListener('click', event => {
    ...
    if (panelButton) { switchPanel(panelButton.dataset.panel); return; }
  ...
  scenarioSelect.addEventListener('change', () => { state.scenario = scenarioSelect.value; state.view = 'default'; state.expanded = { ingredients: false, instructions: false, notes: false }; state.removedIds.clear(); state.removedItems = []; state.previewExpanded.clear(); state.removedExpanded = false; state.mediaSelected = 0; state.panelScroll = { import: 0, media: 0 }; state.pending = null; state.message = ''; closeLayer(false); renderCurrentState(); window.scrollTo(0, 0); });
