...
def _video_count(media: dict) -> int:
    return len([node for node in _photo_nodes(media) if node.get("is_video") and node.get("video_url")])


async def _fetch_instagram_embed(raw_url: str, fetch: Fetch, max_images: int) -> tuple[str, str | None, str, list[ImageDescriptor], int]:
    ...
    return normalized_url, _author_name(media), _caption_text(media), descriptors, _video_count(media)
...
class InstagramUrlContentLoader:
    ...
    async def load(self, url: str, max_images: int, max_image_bytes: int) -> LoadedUrlContent:
        try:
            normalized_url, author_name, text, descriptors, video_count = await _fetch_instagram_embed(url, self.fetch, max_images)
            ...
            log_info(
                ...
                detectedVideoCount=video_count,
            )
            return LoadedUrlContent(url=normalized_url, author_name=author_name, text=text or f"URL: {normalized_url}", images=images)
        except Exception as error:
