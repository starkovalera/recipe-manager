...
async def test_video_processor_reports_transcription_failure_and_keeps_poster():
    ...
    assert result.resource_results[1].error == "NETWORK_ERROR"


@pytest.mark.anyio
async def test_video_processor_rejects_explicit_non_video_content_type():
    async def fetch(url: str, _max_bytes: int) -> FetchResponse:
        return FetchResponse(content=b"video", headers={"content-type": "application/json"})
    ...
    result = await processor.prepare_first_pass_video_sources(
        videos=[LoadedRemoteVideo(url="https://cdn.example/video.mp4", poster_url=None, position=0, original_name="video.mp4")],
        max_image_bytes=1000,
        max_video_bytes=1000,
    )

    assert result.resource_results[0].status == SecondaryResourceLoadStatus.SKIPPED
    assert result.resource_results[1].status == SecondaryResourceLoadStatus.FAILED
    assert result.resource_results[1].error == "RESPONSE_TYPE_UNSUPPORTED"
