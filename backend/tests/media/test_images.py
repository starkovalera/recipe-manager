...
def write_context(purpose: StorageUserPurpose) -> StorageUserContext:
    return StorageUserContext(owner_id="owner-1", purpose=purpose, entity_id="job-1")
...
def test_create_cover_image_saves_derivative_without_mutating_source(tmp_path: Path):
    ...
    source = storage.save(
        ...
        "source.jpg",
        "image/jpeg",
        context=write_context(StorageUserPurpose.IMPORT_SOURCE),
    )
    ...
    cover = create_cover_image(
        ...
        StorageLocation.USER_MEDIA,
        source.storage_key,
        context=write_context(StorageUserPurpose.IMPORT_DERIVED),
