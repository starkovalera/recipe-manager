...
def test_recipe_extraction_prompt_keeps_reference_import_rules():
    ...
    assert 'cannot extract enough useful information from the sources to parse a recipe, return {"notARecipe": true}' in prompt
    ...
    assert 'Return "tags" as a list of tags from the following list that align with the primary recipe: десерт, аэрогриль' in prompt
